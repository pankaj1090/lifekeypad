...
class Backend extends CI_Controller {
	...
	public function index(){
        ...
        $data['emaillist_uniq'] = $this->DatabaseModel->access_database('ts_emaillist','groupby','e_email',array('special_groupby','e_email'));
...
$this->DatabaseModel->access_database('ts_eplist','delete','',array('eplist_parentid'=>$resID));
	    ...
	    if(isset($_POST['catename'])) {
            ...
            //print_r($_FILES);
            	if( $_FILES['cateimage']['name'] != '' ) {
					$path=dirname(__FILE__);
					$abs_path=explode('/application/',$path);
					$pathToCate = $abs_path[0].'/repo/categories/';
					
					$this->load->library('image_lib'); 
					$config['upload_path'] = $pathToCate;
					$config['max_size'] = 0;
					$config['allowed_types'] = 'jpg|jpeg|png';
					...
					if ($this->upload->do_upload('cateimage'))
					{
						...
						$config2['source_image'] = $pathToCate.$img_name.$img_ext;
						...
						rename($pathToCate.$img_name.$img_ext, $pathToCate.$imgNewname);
					
						$cateDataArr['cate_image']=$imgNewname;
					
					}
					else {
						$this->session->userdata['ts_error'] = "Please, use JPG , JPEG or PNG type image.";
                ...
                if($_POST['old_cateid']=='0') {
					...
					if(isset($cateDataArr['cate_image'])) { 
						/***** Remove Previous Uploads STARTS **********/
								
						$cate_img_det = $this->DatabaseModel->access_database('ts_categories','select','',array('cate_id'=>$_POST['old_cateid']));
						if( $cate_img_det[0]['cate_image'] != '' ) {
							unlink( $pathToCate.$cate_img_det[0]['cate_image'] );
						}				
	...
	function language_section($lan_id=''){
		if(isset($_POST['lanname'])) {
			$data_arr = array(
				'lancate_name'	=>	$_POST['lanname'],
				'lancate_symbol'	=>	$_POST['lansymbol']
			);
			if( $_POST['old_lanid'] == '0' ) {
				$this->DatabaseModel->access_database('ts_lancate','insert',$data_arr,'');
				$this->session->userdata['ts_success'] = 'Language added successfully.';
			}
			else {
				$this->DatabaseModel->access_database('ts_lancate','update',$data_arr,array('lancate_id'=>$_POST['old_lanid']));
				$this->session->userdata['ts_success'] = 'Language updated successfully.';
			}
		}
		$data['basepath'] = base_url();
		if($lan_id != '') {
	        $data['solo_lancate'] = $this->DatabaseModel->access_database('ts_lancate','select','',array('lancate_id'=>$lan_id));
	    }
	    else {
	        $data['solo_lancate'] = array();
	    }
	    $data['language_details'] = $this->DatabaseModel->access_database('ts_lancate','select','','');
	    
        $this->load->view('backend/include/header',$data);
        $this->load->view('backend/language_page',$data);
        $this->load->view('backend/include/footer',$data);
	}
