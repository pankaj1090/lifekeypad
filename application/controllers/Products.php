...
class Products extends CI_Controller {
				...
				if( $_POST['prod_type'] == 'Other' ) {
					...
					if($this->upload->do_upload('file')){
						...
						if( !file_exists( $finaldirectory ) ) {
							...
							$this->db->select('*');
							$this->db->from('ts_prodgallery');
							$this->db->where('prodgallery_pid',$prodId);
							$this->db->like('prodgallery_img', $zip_ext);
							$check = $this->db->get()->result_array();
							if(!empty($check)) {
								$this->DatabaseModel->access_database('ts_prodgallery','delete','', array('prodgallery_id'=>$check[0]['prodgallery_id']));
								unlink($finaldirectory.'/'.$check[0]['prodgallery_img']);
							}
						}
    ...
    function self_product_download($uniqid='') {
        if($uniqid != '') {
            ...
            if( $productDetails[0]['prod_filename'] != '' ) {
