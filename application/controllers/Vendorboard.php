...
//print_r($_SESSION);
        ...
        $data['page_title'] = $this->ts_functions->getlanguage('welcometext','vendorboard','solo');
		$this->load->view('vendor/include/vheader',$data);
		$this->load->view('vendor/include/menu_header',$data);
	...
	/******* Index page ENDS ***************/
	 ...
	 public function manage_products(){
		...
		$data['page_title'] = $this->ts_functions->getlanguage('manageproductsmenu','vendorboard','solo');
		$this->load->view('vendor/include/vheader',$data);
		$this->load->view('vendor/include/menu_header',$data);
	...
	public function add_products_1(){
		if( isset($_POST['p_type']) ) {
			...
			$dataArr = array(
				...
				'prod_readby' =>  $_POST['p_readby'],
				'prod_writter' =>  $_POST['p_writter'],
				'prod_runtime' =>  $_POST['p_runtime'],
				'prod_recording' =>  $_POST['p_recording'],
				'prod_publisher' =>  $_POST['p_publisher'],
				'prod_language' =>  $_POST['p_language'],
				'prod_isbn' =>  $_POST['p_isbn']
			);
		...
		$data['page_title'] = $this->ts_functions->getlanguage('addprodstep1text','vendorboard','solo');
		$this->load->view('vendor/include/vheader',$data);
		$this->load->view('vendor/include/menu_header',$data);
	...
	public function add_products_2($prod_id=''){
		...
		$data['page_title'] = $this->ts_functions->getlanguage('aphead2text','vendorboard','solo');
		$this->load->view('vendor/include/vheader',$data);
		$this->load->view('vendor/include/menu_header',$data);
	...
	function upload_prod_files(){
		if(isset($_FILES['file'])) {
			...
			$pathToSampleEbook = $abs_path[0].'/repo/sampleebook/';
			...
			elseif( $_POST['prod_name'] == 'Product.zip' ) {
              ...
              elseif( $_POST['prod_name'] == 'Sample_Ebook' ) {
				// Sampe Ebook Zip
				
				$config1['upload_path'] = $pathToSampleEbook;
				$config1['allowed_types'] = '*';
				$config1['max_size'] = 0;
				$this->load->library('upload', $config1);
				$this->upload->initialize($config1);
				...
				/***** Remove Previous Uploads STARTS **********/
					...
					$prod_upload_det = $this->DatabaseModel->access_database('ts_products','select','',array('prod_id'=>$_POST['prod_id']));
					if( $prod_upload_det[0]['prod_sample'] != '' ) {
						if( strpos($prod_upload_det[0]['prod_sample'],'/') === false ) {
							unlink( $pathToSampleEbook.$prod_upload_det[0]['prod_sample'] );
						}
					}				
				/***** Remove Previous Uploads ENDS **********/
					...
					rename($pathToSampleEbook.$zip_name.$zip_ext, $pathToSampleEbook.$zipNewname);
					$imgDataArr['prod_sample']=$zipNewname;
					$this->DatabaseModel->access_database('ts_products','update',$imgDataArr,array('prod_id'=>$_POST['prod_id']));
					echo '1';
				}
				else {
	...
	public function update_products($uniq_id=''){
		...
		$data['page_title'] = $this->ts_functions->getlanguage('updateprodstep1text','vendorboard','solo');
		$this->load->view('vendor/include/vheader',$data);
		$this->load->view('vendor/include/menu_header',$data);
		$this->load->view('vendor/add_products_step1',$data);
		$this->load->view('vendor/include/vfooter',$data);
    ...
    public function view_products($uniqid=''){
        if($uniqid != '') {
			...
			$data['page_title'] = $productdetails[0]['prod_name'];
			$this->load->view('vendor/include/vheader',$data);
			$this->load->view('vendor/include/menu_header',$data);
    ...
    function sales_history(){
		...
		$data['page_title'] = $this->ts_functions->getlanguage('statementtext','vendorboard','solo'); 
	...
	public function wallet_statement() {
		...
		$data['page_title'] = $this->ts_functions->getlanguage('walletmenu','vendorboard','solo');
		$this->load->view('vendor/include/vheader',$data);
		$this->load->view('vendor/include/menu_header',$data);
