...
class Ts_functions {
    ...
    ****/
     ...
     public function getUrlName($name=''){
        if($name!= ''){
                $p = strtolower($name);
                $p = str_replace(' ','-',$p);
                $p = preg_replace('!-+!', '-', $p);
    ...
    ****/
	...
	/****
        getlanguage : Function to get vendor display box,
        First Param : user db id
    ****/
	public function getVendorDisplayboxbackup($uid=''){
        if($uid != ''){
        	
        	$userDetail = $this->CI->DatabaseModel->access_database('ts_user','select','',array('user_id'=>$uid));
            if(!empty($userDetail)) {
            	$countryDetail = $this->CI->DatabaseModel->access_database('ts_country','select','',array('countryId'=>$userDetail[0]['user_country']));
            	$country_name = !empty($countryDetail) ? $countryDetail[0]['countryName'] : '' ;
				
				if( $userDetail[0]['user_pic'] != '') {
					return '<img src ="'.base_url().'webimage/'.$userDetail[0]['user_pic'].'"> <p>'.$userDetail[0]['user_uname'].'</p> <span>'.$country_name.'</span>';
				}
				else {
					return '<img src ="'.base_url().'webimage/dummy_testi.jpg"> <p>'.$userDetail[0]['user_uname'].'</p> <span>'.$country_name.'</span>';
				}
            }
            else {
                return '';
...
public function getVendorDisplaybox($uid=''){
        if($uid != ''){
            ...
            if(!empty($userDetail)) {
                ...
                if( $userDetail[0]['user_pic'] != '') {
                   return '<img src="'.base_url().'webimage/'.$userDetail[0]['user_pic'].'" class="img-responsive" alt="">
                        <div class="lk_namediv">
                            <a href="'.base_url().'timeline/'.$userDetail[0]['user_uname'].'/'.$userDetail[0]['user_id'].'"><h4>'.$userDetail[0]['user_uname'].' <span class="lk_location">'.$country_name.'</span></h4><a>
                        </div>';
                }
                else {
                    return '<img src="'.base_url().'webimage/dummy_testi.jpg" class="img-responsive" alt="">
                        <div class="lk_namediv">
                          <a href="'.base_url().'timeline/'.$userDetail[0]['user_uname'].'/'.$userDetail[0]['user_id'].'">  <h4>'.$userDetail[0]['user_uname'].' <span class="lk_location">'.$country_name.'</span></h4></a>
                        </div>';
                }
    ...
    public function getVendorImage($uid=''){
        if($uid != ''){
  ...
  public  function select_single_data($table, $where='',$column){
      ...
      public function disprice($prod_price) { 
        $discount=$this->getsettings('commission','discount');
        $site_comis = $this->getsettings('vendor','commission');
        $prod_comis= ($prod_price*$site_comis)/100;
        $vc=$prod_price-$prod_comis;
        $admin_discount=($prod_comis*$discount)/100;
        $ac=$prod_comis-$admin_discount;
        return round($vc+$ac,2);
    } 
...
function checked_like($post_id){
    if(isset($this->CI->session->userdata['ts_uid']) ) {
        $uid=$this->CI->session->userdata['ts_uid'];
        $chk=$this->CI->my_model->select_data('lk_id','ts_likes',array('lk_post_id'=>$post_id,'lk_uid'=>$uid));
        if(!empty($chk)){
            return 'liked';
        }else{
            return '';
...
/******  Start product picture *****/
 public function getProductPic($prodid='',$small=''){
        $prodPic=base_url().'themes/'.$this->current_theme().'images/white_image.jped';
        if($prodid != ''){
            $res = $this->CI->my_model->select_data('prod_image','ts_products',array('prod_id'=>$prodid));
            if(!empty($res)) {
                if($res[0]['prod_image']!=''){
                   $prodName = $this->getProductName($prodid);
                    $image_a = explode('.',$res[0]['prod_image']);
                    if($small==''){
                    $dis_img = 'small/'.$image_a[0].'_thumb.'.$image_a[1];
                   }else{
                    $dis_img = $res[0]['prod_image'];
                   }

                    $prodPic=base_url().'repo/images/'.$dis_img;
                }
            }
        }
        return $prodPic;die;
